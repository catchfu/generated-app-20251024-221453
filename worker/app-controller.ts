...
export class AppController extends DurableObject<Env> {
  ...
  // --- Leaderboard Methods ---
  async addLeaderboardEntry(date: string, entry: { name: string; score: number }): Promise<void> {
    const key = `leaderboard_${date}`;
    const leaderboard = await this.ctx.storage.get<LeaderboardEntry[]>(key) || [];
    ...
    await this.ctx.storage.put(key, topScores);
  }
  async getLeaderboardForDay(date: string): Promise<LeaderboardEntry[]> {
    const key = `leaderboard_${date}`;
    const leaderboard = await this.ctx.storage.get<LeaderboardEntry[]>(key) || [];
    return leaderboard;
  }
